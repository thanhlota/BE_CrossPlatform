...
router.post("/set_comment", verify, async (req, res) => {
  var { id, comment, token } = req.query;
  if (!id || !comment || !token) {
    console.log("No have parameter id, comment, token");
  ...
  try {
    ...
    res.status(200).send({
      ...
      data: {
        id: savedComment._id,
        comment: savedComment.comment ? savedComment.comment : null,
        created: TimeDisplay(savedComment.created),
  ...
  try {
    ...
    res.status(200).send({
      ...
      data: sliceComments.map((comment) => {
        if (is_blocked(user, comment.poster))
          return {
            id: comment._id,
            comment: comment.comment ? comment.comment : null,
            created: TimeDisplay(comment.created),
            poster: comment.poster
              ? {
                  id: comment.poster._id,
                  name: comment.poster.name ? comment.poster.name : null,
                  avatar: comment.poster.avatar.url
                    ? comment.poster.avatar.url
                    : null,
                }
              : null,
